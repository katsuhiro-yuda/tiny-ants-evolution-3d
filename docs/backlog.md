...
### Phase 0 時点のまま更新されていないコメントが残っている

- 検出: Phase 1 後 / ソース解説中に発見
- 内容: 「Phase 1 で追加する」と書かれたまま、実際には Phase 1 で実装済みの箇所がある
  - `world/scene/WorldScene.tsx` — 「水場・岩・植物はPhase 1で資源として追加する」→ `TerrainFeatures.tsx` / `FoodInstances.tsx` として実装済み
  - `world/camera/CameraRig.tsx` — 「選択個体の追跡はPhase 1で個体選択を実装してから追加する」→ 個体選択は実装済み。ただし**カメラ追跡自体は未実装**（仕様書 §9「選択個体を追跡する機能」）のため、この記述は前半だけが古い
  - `simulation/config/world.ts` — `PHASE0_ANT_SPEED_MIN/MAX` のコメント（別項目で記録済み）
- 影響: 読んだ人が「まだ実装されていない」と誤解する。逆に CameraRig は「未実装の機能がある」ことが埋もれている
- 対応時期: **Phase 2 着手時**
- 方針: コメントを現状に合わせる。カメラ追跡は未実装の機能として backlog へ独立させるか、Phase 2 の Issue に含める

### 選択個体のカメラ追跡が未実装

- 検出: Phase 1 後 / ソース解説中に発見
- 内容: 仕様書 §9「選択個体を追跡する機能」、§10 個体詳細の「追跡開始、解除」が未実装。`CameraRig.tsx` のコメントに「Phase 1で個体選択を実装してから追加する」とあるが、個体選択を実装した後も追加されていない
- 影響: 蟻は画面上で数ピクセルしかないため、選択しても目で追えない。実用上の不便がある
